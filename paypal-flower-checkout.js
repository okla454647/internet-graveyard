...
let current=null,currentId=0,fillHooked=false,openHooked=false,renderSeq=0,watchedModal=null;
...
function graveId(){const a=Number(current?.id);if(a)return a;if(currentId)return currentId;const dlg=document.getElementById('detailDialog');const ds=Number(dlg?.dataset?.graveId||dlg?.dataset?.id);return ds||0}
let sdkPromise=null;
function loadSdk(){
 if(window.paypal?.Buttons)return Promise.resolve();
 if(sdkPromise)return sdkPromise;
 sdkPromise=new Promise((ok,bad)=>{
  const done=()=>window.paypal?.Buttons?ok():bad(new Error('PayPal SDK loaded but Buttons unavailable'));
  let s=document.querySelector('script[data-ig-paypal]');
  if(s){
   if(window.paypal?.Buttons)return ok();
   s.addEventListener('load',done,{once:true});s.addEventListener('error',()=>bad(new Error('PayPal SDK network error')),{once:true});
   setTimeout(()=>{if(window.paypal?.Buttons)ok()},500);
   return;
  }
  s=document.createElement('script');s.dataset.igPaypal='1';s.async=true;
  s.src='https://www.paypal.com/sdk/js?client-id='+encodeURIComponent(CLIENT_ID)+'&currency=USD&intent=capture&components=buttons&enable-funding=card';
  s.onload=done;s.onerror=()=>bad(new Error('PayPal SDK network error'));document.head.appendChild(s);
 });
 return sdkPromise;
}
...
function ensureHost(){const d=document.getElementById('ig-flower-gift-modal');if(!d?.open)return null;let h=d.querySelector('#ig-paypal-host');if(!h){h=document.createElement('div');h.id='ig-paypal-host';h.style.cssText='margin-top:12px;min-height:48px';const pay=d.querySelector('.ig-fg-pay');pay?.insertAdjacentElement('afterend',h);if(pay)pay.style.display='none'}return h}
async function render(){
 const host=ensureHost();if(!host)return;
 const seq=++renderSeq;host.innerHTML='<div style="padding:12px;text-align:center;color:#aaa;font-size:11px">'+(zh()?'正在載入 PayPal…':'Loading PayPal…')+'</div>';
 try{
  await loadSdk();if(seq!==renderSeq||!host.isConnected)return;
  host.innerHTML='';
  if(!window.paypal?.Buttons)throw new Error('PayPal Buttons unavailable');
  ...
  if(buttons.isEligible&&!buttons.isEligible())throw new Error('PayPal Buttons not eligible');
  await buttons.render(host);
 }catch(e){console.error(e);if(host.isConnected)host.innerHTML='<div style="padding:11px;border:1px solid #633;border-radius:9px;color:#e0aeb8;font-size:11px">'+(zh()?'PayPal 載入失敗：':'PayPal failed to load: ')+(e?.message||String(e))+'</div>'}
}
function bindModal(d){if(!d||watchedModal===d)return;watchedModal=d;new MutationObserver(()=>{if(d.open)setTimeout(render,30)}).observe(d,{attributes:true,attributeFilter:['open']});d.addEventListener('click',e=>{if(e.target.closest('.ig-fg-card'))setTimeout(render,30)});if(d.open)setTimeout(render,0)}
function start(){hookDetail();loadSdk().catch(e=>console.error('PayPal preload',e));const observer=new MutationObserver(()=>{hookDetail();bindModal(document.getElementById('ig-flower-gift-modal'))});observer.observe(document.documentElement,{childList:true,subtree:true});bindModal(document.getElementById('ig-flower-gift-modal'));setInterval(()=>{hookDetail();bindModal(document.getElementById('ig-flower-gift-modal'))},500)}
